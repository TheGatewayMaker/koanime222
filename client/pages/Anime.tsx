...
export default function AnimePage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [i, e] = await Promise.all([fetchAnimeInfo(id), fetchEpisodes(id)]);
        setInfo(i);
        setEpisodes(e);
      } finally {
        setLoading(false);
              ...
              <div className="flex flex-col gap-6 md:flex-row">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-sm text-foreground/70">
                    ...
                    {info.rating != null && <span className="ml-2 rounded bg-black/30 px-2 py-0.5 text-xs">⭐ {info.rating.toFixed(1)}</span>}
                  </div>
                  {info.genres && info.genres.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {info.genres.map((g) => (
                        <span key={g} className="rounded bg-accent px-2 py-1 text-xs">{g}</span>
                      ))}
                    </div>
                  )}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {episodes.map((ep) => (
                <button
                  ...
                  onClick={() => toast("Streaming Error", { description: "Playback is not available yet.", duration: 3000 })}
                >
